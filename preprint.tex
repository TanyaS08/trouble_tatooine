...
\noindent {\LARGE \textbf{\textsf{Trouble on Tatooine: constructing a
plausible fantasy food web}}}
...
Tanya Strydom --- \texttt{tanya.strydom@umontreal.ca}\\
}
\end{flushleft}
...
food web\\transfer learning\\fantasy ecology\\}
}




        {\bfseries Purpose:}\,A half-baked experiemnt on trying to
construct a food web for Tatooine using real world data.\\%
        {\bfseries Motivation:}\,I'm going through a Star Wars
moment™.\\%
        {\bfseries Conceptally:}\,What if we're dealing with realy poor
data coveragw for a location can we still salvage \emph{something}\\%
        {\bfseries Plan:}\,Take a species list for Tatooine, assign some
traits and use that as the transfer medium from real-world trait and
interaction data.\\%
        {\bfseries Outcome?:}\,Maybe we can also identify some missing
species based on trophic levels/links are missing\\%
        {\bfseries Challenges:}\,What to treat as a complete species
list for Tatooine, reconciling the fact that species were developed
based on `narrative need' as opposed to 'ecologcal need\\%
...
\texttt{@Elton1927AniEco} produces (\textbf{Elton1927AniEco?}), and
\texttt{{[}@Camerano1880EquViv{]}} produces
(\textbf{Camerano1880EquViv?}).
